class EmployeeResignationsController < ApplicationController
  ...
  def first_approve
    @employee_resignation = EmployeeResignation.find(params[:format])
      @employee_resign = EmployeeResignation.where(id: @employee_resignation.id)
    if @employee_resignation.employee.manager_2_id.nil?
      @employee_resign = EmployeeResignation.where(id: @employee_resignation.id)
      @employee_resign.update_all(is_pending: true,is_first_approved: true,is_second_approved: true,resign_status: "SecondApproved")
      ...
      @employee_resign.update_all(is_pending:true,is_first_approved: true,second_reporter_id: @employee_resignation.employee.manager_2_id,resign_status: "FirstApproved",is_second_approved: false,is_second_rejected: false, is_cancelled: false)
