...
class EmployeeLeavRequestsController < ApplicationController
  ...
  def leave_request_excel
    ...
    respond_to do |format|
      format.xls {render template: 'employee_leav_requests/leave_request.xls.erb'}
    end
  end

  def leave_request_pdf
    @start_date = params[:start_date]
    @end_date = params[:end_date]
    @company_id = params[:company_id]
    @location = params[:company_location_id]
    ...
    respond_to do |format|
      format.json
      format.pdf do
        render pdf: 'employee_leav_request',
              layout: 'pdf.html',
  ...
  def status_wise_excel
    ...
    respond_to do |format|
      format.xls {render template: 'employee_leav_requests/status_wise.xls.erb'}
    end
  end
