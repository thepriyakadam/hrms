...
3.upto(370) do |line|
  ...
  unless workingday.nil?
    workingday.cl_leave = ex.cell(line,'D').to_f
    workingday.el_leave = ex.cell(line,'E').to_f

    workingday.cl_balance = ex.cell(line,'F').to_f
    workingday.el_balance = ex.cell(line,'G').to_f
    workingday.save
  end
end