class DailyAttendance < ActiveRecord::Base
  ...
  def self.fetch_data(day)
    ...
    punch.each do |mat|
      ...
      user_i = mat.UserID
      user_split = user_i.match /(\d{2})(\d{3})/
  ...
  def self.third_fetch_data(day)
    punch = ThirdDatabase.where("Edatetime > ?", Time.now - day.days)
    punch.each do |mat|
      punch_date_time = mat.Edatetime
      puts "Id --- #{mat.UserID}"
      ...
      puts "----------#{user_i}"
       ...
       #user_id = split.second
       #puts "Split number"
       @employee = Employee.find_by_manual_employee_code(user_i)
       if @employee.nil?
         puts "Employee ID is----------------- #{@employee} not present in our HRMS"
       else
        employee_id = @employee.id
        puts "employee att working emp id is #{employee_id}"
        employee_first_name = @employee.first_name
        employee_last_name = @employee.last_name
        space = " "
        if employee_last_name.present?
        ...
        if @employee_att_present.present?
          ...
          emmployee_att_first = EmployeeAttendance.create(employee_id: employee_id, day: punch_date, present: "P", in_time: punch_time, month_name: punch_month, employee_code: user_i, employee_name: employee_name)
          puts "-------EmployeeAttendance created 1 In Time created..#{Time.now.to_date} .. #{Time.now}---------"
        end
      end
    end
