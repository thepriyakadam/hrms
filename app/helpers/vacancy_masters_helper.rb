  ...
  def all_department_type_list2
    if current_user.class == Group
      ...
      if current_user.role.name == 'Company'
        ...
        Department.where(company_location_id: current_user.company_location_id).collect { |d| [d.name, d.id] }
      elsif current_user.role.name == 'Department'
        Department.where(id: current_user.department_id).collect { |d| [d.name, d.id] }
      end
    end
  end
