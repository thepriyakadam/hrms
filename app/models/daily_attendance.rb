class DailyAttendance < ActiveRecord::Base
  ...
  def self.fetch_data(day)
    punch = CheckInOut.where("CHECKTIME > ? ", Time.now - day.days)
    punch.each do |mat|
  ...
  #def self.fetch_data(day)
   # @punch_dates = DailyAttendance.where(date: "2018-06-01" .. "2018-06-11")
   # @punch_dates.each do |punch_date|
   # emp = Employee.find_by_manual_employee_code(punch_date.employee_code)
     # employee_id = emp.id
     # daily_att_day = DailyAttendance.where(employee_code: punch_date.employee_code, date: punch_date.date).order("time ASC")
     # @employee_in_time = daily_att_day.first.time.to_time
     # @employee_out_time = daily_att_day.last.time.to_time
     # @employee_att_present = EmployeeAttendance.where(employee_id: employee_id, day: punch_date.date)
     # emp_att_out_time = @employee_att_present.update_all(in_time: @employee_in_time, out_time: @employee_out_time)
     #   puts "-----------EmployeeAttendance Out Time 1 updated #{Time.now}-----------"
     # end
  #end
  ...
  # def self.check_attendance
  #    @employees = Employee.where(status: "Active")
  #    @employees.each do |e|
  #      @emp_id = e.id
  #      employee_atten = EmployeeAttendance.find_by(employee_id: @emp_id, day: Time.now.to_date)
  #       puts "------ #{employee_atten}"
  #       if employee_atten.nil?
