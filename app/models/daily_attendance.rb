    ...
    calculate_att.each do |cal_att|
      if cal_att.is_confirm == true
        ...
        if @halfday_allow == true #
          ...
          if (@company_time..@company_late_time).include?(@emp_in_time) #Start Within  Time Limit
  ...
  def self.third_fetch_data(day)
    punch = ThirdDatabase.where("RecDate > ?", Time.now - day.days)
      ...
      @employee1 = Employee.find_by_manual_employee_code(user_i)
        ...
        daily_att_employee_present = DailyAttendance.where(employee_code: user_id, time: punch_time)
        if daily_att_employee_present.empty?
          daily_att_employee_create = DailyAttendance.create(employee_code: user_id, date: punch_da$
          puts "-----DailyAttendance Created 0 #{Time.now}---------"
        end
        daily_att_day = DailyAttendance.where(employee_code: user_id, date: punch_date).order("time$
        @employee_in_time = daily_att_day.first.time.to_time
        @employee_out_time = daily_att_day.last.time.to_time
        ...
        if @employee_att_present.present?
          if @employee_att_present.first.in_time.present?
            emp_att_out_time = @employee_att_present.update_all(in_time: @employee_in_time, out_tim$
            puts "-----------EmployeeAttendance Out Time 1 updated #{Time.now}-----------"
          else
            emp_att_out_time = @employee_att_present.update_all(in_time: @employee_in_time, out_tim$
            puts "-----------EmployeeAttendance Out Time 1 updated #{Time.now}-----------"
          ...
          emmployee_att_first = EmployeeAttendance.create(employee_id: employee_id, day: punch_date$
          puts "-------EmployeeAttendance created 1 In Time created..#{Time.now.to_date} .. #{Time.$
        end
      end
    end
  end
  
  def self.third_calculate_attendance(day)
    puts "-----calculate attendance start  #{Time.now.to_date} -- #{Time.now}-------"
    fullday_working_hrs = LatemarkMaster.last.fullday_working_hrs.to_i
    @company_fullday_working_hrs = Time.at(fullday_working_hrs).utc.strftime("%H:%M")
    halfday_working_hrs = LatemarkMaster.last.halfday_working_hrs.to_i
    ...
    calculate_att.each do |cal_att|
      if cal_att.is_confirm == true
        ...
        @employee_out_time = cal_att.out_time
        @att_day = cal_att.day
        employee_in_time_att = EmployeeAttendance.where(employee_id: @employee_id, in_time: @employ$
          ...
          if working_hrs > @company_fullday_working_hrs # Start full Day Attendance
            if cal_att.on_duty_request_id.present? || cal_att.employee_leav_request_id.present?
              employee_in_time_att.update_all(working_hrs: working_hrs)
               ...
               employee_in_time_att.update_all(working_hrs: working_hrs, present: "HP", comment: "H$
            elsif cal_att.employee_week_off_id.present?
               employee_in_time_att.update_all(working_hrs: working_hrs, present: "WOP", comment: "$
            else
              employee_in_time_att.update_all(working_hrs: working_hrs, present: "P", comment: "Ful$
            end
            ...
            if cal_att.on_duty_request_id.present? || cal_att.employee_leav_request_id.present?
              employee_in_time_att.update_all(working_hrs: working_hrs)
               ...
               employee_in_time_att.update_all(working_hrs: working_hrs, present: "HHDP", comment: $
            elsif cal_att.employee_week_off_id.present?
               employee_in_time_att.update_all(working_hrs: working_hrs, present: "WOHDP", comment:$
            else
              employee_in_time_att.update_all(working_hrs: working_hrs, present: "HD", comment: "Ha$
            end
          end # End Full Day Attendance
            ...
            if working_hrs > @company_fullday_working_hrs
              if cal_att.on_duty_request_id.present? || cal_att.employee_leav_request_id.present?
                employee_in_time_att.update_all(working_hrs: working_hrs)
                 ...
                 employee_in_time_att.update_all(working_hrs: working_hrs, present: "HP", comment: $
              elsif cal_att.employee_week_off_id.present?
                 employee_in_time_att.update_all(working_hrs: working_hrs, present: "WOP", comment:$
              else
                employee_in_time_att.update_all(working_hrs: working_hrs, present: "P", comment: "F$
              end
            else
              if cal_att.on_duty_request_id.present? || cal_att.employee_leav_request_id.present?
                employee_in_time_att.update_all(working_hrs: working_hrs)
                 ...
                 employee_in_time_att.update_all(working_hrs: working_hrs, present: "WOHDP", commen$
              else
                employee_in_time_att.update_all(working_hrs: working_hrs, present: "HD", comment: "$
              end
            end
          ...
          if (@company_time..@company_late_time).include?(@emp_in_time) #Start Within  Time Limit
            if cal_att.on_duty_request_id.present? || cal_att.employee_leav_request_id.present? || $
             ...
             if cal_att.on_duty_request_id.present? || cal_att.employee_leav_request_id.present? ||$
              ...
              employee_in_time_att.update_all(working_hrs: working_hrs, present: "HD", comment: "La$
             end
          end
        end
