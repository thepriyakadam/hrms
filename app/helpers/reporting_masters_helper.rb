module ReportingMastersHelper
  ...
  def all_reporting_master_list1
    ReportingMaster.where.not(employee_id: current_user.employee_id).collect { |e| [e.try(:employee).try(:manual_employee_code).to_s + ' ' + e.try(:employee).try(:first_name).to_s + ' ' + e.try(:employee).try(:last_name).to_s, e.id] }
  end
end
